...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N)
    Y = 2 * X + mu + np.random.normal(0, np.sqrt(sigma2), N)  # Replace with code to generate random values for Y with specified mean and variance
    ...
    for _ in range(S):
        ...
        Y_sim = 2 * X_sim + mu + np.random.normal(0, np.sqrt(sigma2), N)  # Replace with code to generate Y values
